...
3. All content of the data directory, except for the `bonkcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `bonkcoin.conf`       | User-defined configuration settings for `bonkcoind` or `bonkcoin-qt`; can be specified by `-conf` option
`./`               | `bonkcoind.pid`        | Stores the process ID (PID) of `bonkcoind` or `bonkcoin-qt` while running; can be specified by `-pid` option
...
`bonkcoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
