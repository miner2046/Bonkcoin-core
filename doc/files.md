...
3. All content of the data directory, except for the `friccoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `friccoin.conf`       | User-defined configuration settings for `friccoind` or `friccoin-qt`; can be specified by `-conf` option
`./`               | `friccoind.pid`        | Stores the process ID (PID) of `friccoind` or `friccoin-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `friccoind` or `friccoin-qt`
...
`friccoin-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
